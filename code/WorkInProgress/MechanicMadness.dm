...
/obj/item/mechanics
	...
	process()
		...
		if(particless.len != mechanics.connected_outgoing.len)
			cutParticles()
			for(var/atom/X in mechanics.connected_outgoing)
				particless.Add(particleMaster.SpawnSystem(new /datum/particleSystem/mechanic(src.loc, X.loc)))
